...
def estimate(model,solver,data,theta0=[0.00001,0000.1,0.00001],twostep=0):
    ...
    tabulate1 = data1.dx1.value_counts() #Count number of observations for each dx1
    ...
    model.p1[:] = p1
    model.p2[:] = p2 # Use first step estimates as starting values for p
...
def ll(theta, model, solver,data, pnames, out=1, no_guess = False): # out=1 solve optimization
    ...
    x = np.array(data.x ) # x is the index of the observed state
    ...
    choice_prob = lik_pr * (1 - d) * (model.p1[1]*dx1 + model.p1[0]*(1-dx1)) + (1-lik_pr) * d *(model.p2[1]*dx1 + model.p2[0]*(1-dx1)) # get probability of making observed choice
    log_lik = np.log(choice_prob)  # Compute log-likelihood-contributions
    
    # add on log like for mileage process
    if theta.size>3: # theta > 3 if there are parameters for p
...
def score(theta, model, solver, data, pnames):
    ...
    dc2 = 2*theta[2]*(model.grid**2) # Get derivative of cost function in utility wrt eta3
    ...
    score = np.zeros((N, 3 + n_p)) # Initialize score function
    ...
    if theta.size>3:
        for i_p in range(n_p): 
            score[:,3+i_p] = score[:,3+i_p]+invp[dx1,i_p]
...
def updatepar(par,parnames, parvals):
    ...
    for i,parname in enumerate(parnames):
        ...
        if i<3:
            ...
            list_val = [None]*(parvals.size-3) 
            for j,parval in enumerate(parvals[3:]):
                list_val[j]=parval
